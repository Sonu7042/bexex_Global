...
import { LearnMoreButton, HeadingComponent } from "../components/Buttons.jsx";
...
// second filter
const [selectedSecondFilter, setSelectedSecondFilter] = useState("--Select--");
...
const safetyServices =
  selectedSecondFilter === "--Select--"
    ? []                                   // ✅ UPDATED: no third filter options
    : filterDataforthirdfilter.filter(
        (item) =>
          item.category.toLowerCase() ===
          selectedSecondFilter.toLowerCase()
      );

  // third filter
  const [selectedthirdFilter, setselectedthirdFilter] = useState("--Select--");
  // console.log(selectedthirdFilter, "selectedthirdFilter")

  useEffect(() => {
  if (selectedSecondFilter === "--Select--") {
    setselectedthirdFilter("--Select--");          // ✅ UPDATED
    return;
  }
...
const filteredServices = servicesCardData.filter((item) => {
  const matchTitle =
    selectedSecondFilter === "--Select--" ||
    normalize(item.title) === normalize(selectedSecondFilter); // ✅ UPDATED

  const matchCategory =
    selectedthirdFilter === "--Select--" ||
    normalize(item.category) === normalize(selectedthirdFilter); // ✅ UPDATED
...
export default AllServiceShowPage;
